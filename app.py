...
def page_ressource_externe():
    style.section_title("🌐 Ressource externe")
    st.write("Cette ressource s'ouvre dans un nouvel onglet.")
    st.link_button("↗ Ouvrir la ressource externe", RESSOURCE_EXTERNE_URL, type="primary")
...
def main():
    ...
    with st.sidebar:
        style.section_title("🧭 Espace")
        espace = st.radio(
            "Espace", ["📋 Réclamations", "📈 KPI", "🌐 Ressource externe"],
            key="espace_select", label_visibility="collapsed",
        )
        st.divider()

    role_label = "🛠 Admin" if is_admin else "🔎 Consultation"

    if espace == "📋 Réclamations":
        ...
        style.header("Ressource externe", "Accès à la plateforme de référence", role_label)
        page_ressource_externe()
